## Rebuttal
	...
	- ### Response to AkJH {{renderer :wordcountchar_}}
		...
		- **"how does OptiFluence compare to label‑only MIAs at low FPR?"** Can the reviewer elaborate for their expectations? Label-only MIA assumes no access to confidence scores (only predicted labels) This has potential implications for the optimized canary. Does the reviewer want us to incorporate this constraint in the optimization of the canaries? or is the ask to run a completely different MIA than that of Aerni 2024?
		-
- ## Reviews
	- ### Reviewer_AkJH
